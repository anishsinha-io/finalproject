...
int format(int order, size_t node_size) {
    ...
    status = write_buffer(TEST_INDEX_PATH, &h, sizeof(h), 0);
    if (status != 0) return -1;
    status = write_buffer(TEST_INDEX_PATH, root, sizeof(*root), sizeof(header));
    if (status != 0) return -1;
    return 0;
}
...
*/
int destroy_index() {
    return clear_file(TEST_INDEX_PATH);
}
...
int read_header(header *h) {
    return read_buffer(TEST_INDEX_PATH, h, sizeof(*h), 0);
}
...
int write_header(header *h) {
    return write_buffer(TEST_INDEX_PATH, h, sizeof(*h), 0);
}
...
int read_node(node *n, int loc) {
    ...
    return read_buffer(TEST_INDEX_PATH, n, sizeof(*n), offset);
}
...
int write_node(int loc, node *n) {
    header h;
    read_header(&h);
    write_buffer(TEST_INDEX_PATH, n, sizeof(*n), get_offset(loc));
